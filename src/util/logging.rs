...
use chrono::{Duration, DateTime, Utc, Datelike, TimeZone};
use colored::Colorize;
use once_cell::sync::Lazy;

use std::{sync::Mutex, cell::Cell};

static DAY: Lazy<Mutex<Cell<DateTime<Utc>>>> = Lazy::new(|| {Mutex::new(Cell::new(Utc.with_ymd_and_hms(2000, 01, 01, 0, 0, 0).unwrap()))});
