...
impl<T> From<T> for GovanError
where
	T: Reportable + 'static,
{
	fn from(value: T) -> Self {
		let msgs = value.get_messages();

		GovanError(GovanErrorImpl {
			log_type: LogType::Error,
			to_log: msgs.to_logger.or_else(|| Some(format!("{:?}", value))),
			to_user: match msgs.to_user {
				None => UserMsgType::None,
				Some(s) => UserMsgType::Normal(s),
			},
...
impl From<anyhow::Error> for GovanError {
